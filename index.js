...
(function () {
  ...
  // 배경 스크롤만 막는 최소한의 잠금. 오버레이 자체 크기/위치는
  // CSS의 position:fixed + svh 단위에 맡깁니다 (JS로 픽셀을 강제
  // 계산해서 넣는 방식은 주소창 표시/숨김 타이밍과 꼬여 오히려
  // 위/아래가 잘리는 문제를 만들었습니다).
  let savedScrollY = 0;
  ...
  function spawnMatchBurst(container) {
    if (!container) return;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const startX = w * (0.08 + Math.random() * 0.84);
      const startY = h * (0.10 + Math.random() * 0.25);
      const dx = (Math.random() - 0.5) * w * 0.8;
      const dy = h * (0.35 + Math.random() * 0.55);
